 ...
 * generates `_HSX` shadow classes so script classes can extend Haxe classes.
 * See LICENSE and NOTICE for details.
 */
package crowplexus.hscript.macros;
...
class ClassExtendMacro {
	...
	public static function init() {
		...
		for(apply in Config.ALLOWED_CUSTOM_CLASSES) {
			var skip = false;
			for(r in registered)
				if (apply.startsWith(r)) { skip = true; break; }
			if (skip) continue;
			registered.push(apply);
			Compiler.addGlobalMetadata(apply, "@:build(crowplexus.hscript.macros.ClassExtendMacro.build())");
		}
		#end
		#end
	}
	...
	public static function build():Array<Field> {
		...
		if (!cl.name.endsWith("_Impl_") && !cl.name.endsWith(CLASS_SUFFIX) && !cl.name.endsWith("_HSC")) {
			...
			if(fkey == "crowplexus.hscript.CustomClassHandler.TemplateClass") return fields; // Error: Redefined
			...
			shadowClass.kind = TDClass({
				...
				{name: "IHScriptCustomBehaviour", pack: ["crowplexus", "hscript"]}
			], false, true, false);
			shadowClass.name = '${cl.name}$CLASS_SUFFIX';
			// keep the shadow class reachable via Type.resolveClass at runtime
			shadowClass.meta.push({name: ":keep", params: [], pos: Context.currentPos()});
